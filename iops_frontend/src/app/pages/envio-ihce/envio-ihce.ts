...
export class EnvioIhce implements OnInit {
  ...
  cargarCatalogos() {
    ...
    forkJoin({
      ...
      estadosEnvio: this.envioIhceService.getIhceCatEstadosEnvio().pipe(
        catchError(error => { console.error('Error cargando Estados', error); return of([]); })
      ),
      tiposRda: this.envioIhceService.getIhceCatTiposRda().pipe(
        catchError(error => { console.error('Error cargando Tipos RDA', error); return of([]); })
      )
    }).subscribe({
      next: (responses: any) => {
  ...
  allAllowedSelected(): boolean {
    ...
    return allowedRecords.every((allowed: any) =>
      selected.some(s => s.ingreso === allowed.ingreso)
    );
  }
  ...
  enviarFila(ingreso: any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.envioIhceService.enviarRdaPaciente(payload)
          .pipe(
  ...
  getSeverityStatus(estadoId: number | null): 'success' | 'secondary' | 'info' | 'warn' | 'danger' | 'contrast' {
    ...
    switch (Number(estadoId)) {
      ...
      case 2: return 'success'; // Exitoso
      case 3: return 'contrast'; // FALLIDO
      case 4: return 'danger'; // RECHAZADO
      default: return 'info';
    }
  }
  ...
  abrirJsonLog(ingresoId: number, tipoEndpoint: string, tipoRdaId?: number): void {
    ...
    this.envioIhceService.obtenerJsonLog(ingresoId, tipoEndpoint, tipoRdaId)
      .pipe(
  ...
  abrirJsonLogModal(
    ...
    this.envioIhceService.obtenerJsonLog(ingresoId, tipoEndpoint, tipoRdaId)
      .pipe(
        finalize(() => this.jsonViewerCargando.set(false))
      )
      .subscribe({
        next: (response) => {
