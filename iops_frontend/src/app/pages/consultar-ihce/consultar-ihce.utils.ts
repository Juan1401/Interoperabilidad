...
export function parseFhirPatient(rawResource: any): PatientData | null {
    ...
    const extractExtensionValue = (exts: any[] | undefined, urlPart: string): string => {
        ...
        return ext?.valueString || ext?.valueCode || ext?.valueCoding?.display || ext?.valueCoding?.code || ext?.valueCodeableConcept?.text || ext?.valueCodeableConcept?.coding?.[0]?.display || '---';
    };
...
export function parseFhirCustodian(rawResource: any): CustodianData | null {
    ...
    // Buscar NIT
    const nitIdentifier = identifiers.find((i: any) => i?.type?.coding?.some((c: any) => c?.code === 'NIT'));

    // Buscar Código de habilitación (CodigoPrestador)
    const repsIdentifier = identifiers.find((i: any) => i?.type?.coding?.some((c: any) => c?.code === 'CodigoPrestador'));

    // Extraer el tipo de organización
    const types = resource?.type || [];
    const orgType = types.find((t: any) => t?.coding?.some((c: any) => c?.system?.includes('ColombianProviderClass')));
...
export function parseFhirPractitioner(rawResource: any): PractitionerData | null {
    ...
    const colIdentifier = identifier?.type?.coding?.find((c: any) => c?.system?.includes('ColombianPersonIdentifier')) ?? identifier?.type?.coding?.[0];
