...
export class ConsultarIhce implements OnInit {
  ...
  cargarCatalogos() {
    ...
    forkJoin({
      tiposDocId: this.envioIhceService.getTiposIdPacientes().pipe(
        catchError(error => { console.error('Error cargando Tipos ID Paciente', error); return of([]); })
      )
    }).subscribe({
      next: (responses: any) => {
  ...
  private cargarInfoPaciente(payload: any) {
    ...
    this.consultarIhceService.consultarPacienteExacto(requestPayload).pipe(
  ...
  private extractPatients(fhirPayload: any): any[] {
    ...
    if (fhirPayload.resourceType === 'Bundle' && Array.isArray(fhirPayload.entry)) {
      return fhirPayload.entry
        .map((e: any) => e?.resource)
        .filter((r: any) => r?.resourceType === 'Patient' || r != null);
    }
    ...
    this.consultarIhceService.consultarRdaPaciente(requestPayload).subscribe({
      next: (response: any) => {
        ...
        if (typeof response === 'string') {
          try { parsedData = JSON.parse(response); } catch (e) { }
        }
  ...
  seleccionarEncuentro(fila: any) {
    ...
    if (title.includes('urgencia')) {
      ...
      this.dialogService.open(ModalRdaHospitalizacionComponent, { header: 'Detalle de Hospitalización', width: '100vw', height: '100vh', modal: true, maximizable: true, closable: true, closeOnEscape: true, styleClass: 'rda-modal', data: { fila: fila } });
    } else {
      this.dialogService.open(ModalRdaConsultaComponent, { header: 'Detalle de Consulta Externa', width: '100vw', height: '100vh', modal: true, maximizable: true, closable: true, closeOnEscape: true, styleClass: 'rda-modal', data: { fila: fila } });
    }
  }

  seleccionarRda(item: any) {
    this.dispararAuditoriaRda(item, 1); // 1 = RDA_PACIENTE
    this.dialogService.open(ModalRdaPacienteComponent, { header: 'Detalle del Resumen de Atención en Salud', width: '100vw', height: '100vh', modal: true, maximizable: true, closable: true, closeOnEscape: true, styleClass: 'rda-modal', data: { fila: item } });
  }
