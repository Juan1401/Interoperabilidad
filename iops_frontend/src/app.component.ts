        ...
        @if (isInitialProcessing()) {
            <div class="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-surface-900/70 backdrop-blur-sm transition-opacity duration-300">
                ...
                @if (errorMessage()) {
                    ...
                    <button (click)="retryInit()" class="mt-6 px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-lg hover:bg-primary-emphasis transition-all flex items-center gap-2"><i class="pi pi-refresh"></i> Reintentar</button>
                }
            </div>
        }
        ...
        try {
            ...
            this.authService.fetchAndStoreToken().subscribe({
                next: (authRes: any) => {
                    ...
                    this.authService.getSystemVariable('BioEstadistica', 'app', 'IHCE_key_secreta', authToken).subscribe({
                        next: (sysVarRes: any) => {
