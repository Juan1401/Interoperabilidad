...
cp iops_api/.env.example iops_api/.env
...
> [!IMPORTANT]
> **Base de Datos:** El archivo `docker-compose.dev.yml` no incluye un contenedor de PostgreSQL. Debes tener una instancia de PostgreSQL accesible (local o remota) y configurar el `DB_HOST` en tu archivo `iops_api/.env`. Si usas Docker en Windows/Mac, puedes usar `host.docker.internal` para referirte a tu máquina local.
...
- `/iops_frontend`: Código fuente de la aplicación Angular.
- `/iops_api`: Código fuente de la API Laravel.
...
| **PHP** | `iops_api/docker/php/custom.ini` | **900s** | `max_execution_time` — tiempo máximo de ejecución de un script |
| **Laravel → IHCE** | `iops_api/config/services.php` + `.env` | **120s** | `IHCE_TIMEOUT` — timeout HTTP hacia el Ministerio de Salud |
| **Laravel → SIIS** | `iops_api/app/Services/Hl7/RdaService.php` | **60s** | Petición HTTP al servidor SIIS Legacy (Epicrisis) |
| **Browsershot** | `iops_api/app/Services/Hl7/RdaService.php` | **~30s** | `waitUntilNetworkIdle()` — Chromium renderizando HTML a PDF |

### PHP (`custom.ini`)

La configuración personalizada de PHP se encuentra en [`iops_api/docker/php/custom.ini`](./iops_api/docker/php/custom.ini) y se copia al contenedor durante el build del Dockerfile:
...
1. **PHP:** Editar `iops_api/docker/php/custom.ini` y hacer rebuild de la imagen (`docker compose build api`).
2. **Nginx:** Editar `nginx/nginx-dev.conf` o `nginx-prod.conf` y reiniciar el contenedor Nginx.
3. **IHCE:** Cambiar `IHCE_TIMEOUT` en `iops_api/.env` y ejecutar `php artisan config:clear` dentro del contenedor.
...
- Los logs y tests de Backend se guardan en `iops_api/cache`.
- Los logs y tests de Frontend se guardan en `iops_frontend/cache`.
