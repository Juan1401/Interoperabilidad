...
function colorPorNivel(nivel: string): string {
  const map: Record<string, string> = {
    'I': '#dc2626', '1': '#dc2626', '01': '#dc2626', // Rojo
    'II': '#ea580c', '2': '#ea580c', '02': '#ea580c', // Naranja
    'III': '#ca8a04', '3': '#ca8a04', '03': '#ca8a04', // Amarillo
    'IV': '#16a34a', '4': '#16a34a', '04': '#16a34a', // Verde
    'V': '#2563eb', '5': '#2563eb', '05': '#2563eb', // Azul
  };
...
export function parseFhirTriaje(rawResource: any): TriajeData | null {
  ...
  if (nivelCode === '---' && Array.isArray(resource?.component)) {
    const triageComp = resource.component.find((c: any) =>
      c?.code?.coding?.[0]?.display?.toLowerCase().includes('triai') ||
      c?.code?.text?.toLowerCase().includes('triai')
    );
    if (triageComp) {
...
export function parseFhirDocumentoAdjunto(rawResource: any): string | null {
  ...
  const base64 = resource?.resourceType === 'DocumentReference' 
    ? resource?.content?.[0]?.attachment?.data 
    : (resource?.raw_body && resource?.encoding === 'base64' ? resource.raw_body : null);
