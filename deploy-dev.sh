...
if [ "$BUILD_BACKEND" = true ]; then
    ...
    docker exec hl7restapi_dev mkdir -p storage/framework/sessions
    docker exec hl7restapi_dev mkdir -p storage/framework/views
    docker exec hl7restapi_dev mkdir -p storage/framework/testing

    # Permisos (ahora que los directorios existen)
    echo "🔒 Configurando permisos globales y en storage/cache..."
    docker exec hl7restapi_dev chown -R www-data:www-data /var/www/html
    docker exec hl7restapi_dev chmod -R 755 /var/www/html
    docker exec hl7restapi_dev chmod -R 775 storage bootstrap/cache
    docker exec hl7restapi_dev chmod -R 777 storage/logs storage/framework

    # Instalar dependencias con timeout extendido
    echo "📦 Instalando dependencias de Composer (puede tardar varios minutos)..."
    docker exec hl7restapi_dev bash -c "COMPOSER_PROCESS_TIMEOUT=600 composer install --no-interaction"

    # Instalar dependencias de NPM (Necesario para Browsershot / Puppeteer)
    echo "📦 Instalando dependencias de NPM en Backend..."
    docker exec hl7restapi_dev bash -c "npm install"
    ...
    docker exec hl7restapi_dev chown -R www-data:www-data storage bootstrap/cache
    docker exec hl7restapi_dev chmod -R 777 storage/framework

    # Limpiar cachés (Laravel maneja esto de forma segura)
    echo "🧹 Limpiando cachés..."
    docker exec -u www-data hl7restapi_dev php artisan config:clear || true
    docker exec -u www-data hl7restapi_dev php artisan cache:clear || true
    docker exec -u www-data hl7restapi_dev php artisan view:clear || true

    # Crear HomeController si no existe (necesario para login)
    echo "🏠 Verificando HomeController..."
    docker exec hl7restapi_dev bash -c "[ ! -f app/Http/Controllers/HomeController.php ] && php artisan make:controller HomeController || echo 'HomeController ya existe'"

    echo "🏠 Verificando ClientController..."
    docker exec hl7restapi_dev bash -c "[ ! -f app/Http/Controllers/ClientController.php ] && php artisan make:controller ClientController || echo 'ClientController ya existe'"

    echo "🔑 Verificando llave de aplicación..."
    if docker exec hl7restapi_dev grep -q "APP_KEY=base64:" .env; then
        echo "✅ La llave de aplicación ya existe, omitiendo generación."
    else
        echo "🔑 Generando llave de aplicación..."
        docker exec hl7restapi_dev php artisan key:generate --force
    fi

    # Base de datos y Migraciones
    echo "🗄️  Configurando base de datos..."
    # Crear directorios necesarios
    docker exec hl7restapi_dev mkdir -p Models public/upload storage/logs
    # Asegurar creación del esquema
    docker exec hl7restapi_dev bash -c "echo 'DB::statement(\"CREATE SCHEMA IF NOT EXISTS ihce\");' | php artisan tinker" || true
    # Correr migraciones
    docker exec hl7restapi_dev php artisan migrate --force
    ...
    if docker exec hl7restapi_dev [ ! -f storage/oauth-private.key ]; then
        echo "🔐 Generando llaves OAuth faltantes..."
        docker exec hl7restapi_dev php artisan passport:keys --force
        docker exec hl7restapi_dev chown www-data:www-data storage/oauth-private.key storage/oauth-public.key
        docker exec hl7restapi_dev chmod 600 storage/oauth-private.key storage/oauth-public.key
    fi

    CLIENTS=$(docker exec hl7restapi_dev php artisan tinker --execute="echo DB::table('oauth_clients')->count();" | tr -d '\r' | grep -oE '^[0-9]+')

    if [ "$CLIENTS" -eq 0 ]; then
        echo "🔐 Generando clientes OAuth..."
        docker exec hl7restapi_dev php artisan passport:install --force
    else
    ...
    if [ "$RUN_SEEDERS" = true ]; then
        ...
        docker exec hl7restapi_dev php artisan db:seed --force || echo "⚠️  No hay seeders configurados o ya fueron ejecutados"
    else
    ...
    docker exec hl7restapi_dev chown -R www-data:www-data public/upload storage
    docker exec hl7restapi_dev chmod -R 775 public/upload
    docker exec hl7restapi_dev chmod -R 777 storage/logs storage/framework/cache storage/framework/sessions storage/framework/views
fi
...
if [ "$BUILD_FRONTEND" = true ]; then
    ...
    docker exec hl7frontend_dev npm install --legacy-peer-deps
fi

echo "✅ ¡Despliegue DEV completado!"
echo "   📡 Proxy Nginx: hl7nginx_dev (puertos 4202 y 8002)"
if [ "$BUILD_BACKEND" = true ]; then
    echo "   - Backend API: http://raziel-dev-api.fciclubnoel.com:8002"
    echo "     (o localmente: http://localhost:8002)"
fi
if [ "$BUILD_FRONTEND" = true ]; then
    echo "   - Frontend App: http://raziel-dev.fciclubnoel.com:4202"
    echo "     (o localmente: http://localhost:4202)"
fi
